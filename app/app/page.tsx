    ...
    <div role="dialog" aria-modal="true" onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 70, background: "rgba(0,0,0,0.4)", display: "grid", placeItems: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 720, background: BRAND.surface, borderRadius: 16, border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 20px 40px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ opacity: 0.7 }}>Ref</div><div style={{ fontWeight: 700 }}>{payment.ref}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>Date</div><div>{date}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>{t.pay.landlord}</div><div>{payment.landlord}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>{t.pay.method}</div><div>{payment.method}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>{t.pay.status}</div><div style={{ fontWeight: 600 }}>{payment.status}</div></div>
...
const RedeemModal: React.FC<{ t: I18n; reward: (typeof rewardsCatalog)[number] | null; onClose: () => void; onConfirm: (denom: number) => void; }> = ({ t, reward, onClose, onConfirm }) => {
  ...
  return (
    <div role="dialog" aria-modal="true" onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 70, background: "rgba(0,0,0,0.4)", display: "grid", placeItems: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 520, background: BRAND.surface, borderRadius: 16, border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 20px 40px rgba(0,0,0,0.25)", padding: 16 }}>
        <div style={{ fontWeight: 700, marginBottom: 10 }}>{t.rewards.choose}: {reward.title}</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {reward.denom.map((d) => (
            <button key={d} onClick={() => setDenom(d)} style={{ padding: "8px 10px", borderRadius: 10, border: `1px solid ${denom === d ? BRAND.primary : BRAND.ring}`, background: denom === d ? "#ecfdf5" : BRAND.surface, fontWeight: 600 }}>
              {formatPKR(d)}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 }}>
          <button onClick={onClose} style={{ padding: "10px 12px", borderRadius: 10, border: `1px solid ${BRAND.ring}`, background: BRAND.surface, fontWeight: 600 }}>{t.rewards.cancel}</button>
          <button onClick={() => denom && onConfirm(denom)} disabled={!denom} style={{ padding: "10px 12px", borderRadius: 10, border: `1px solid ${BRAND.ring}`, background: !denom ? "#f3f4f6" : BRAND.primary, color: !denom ? "#6b7280" : "#fff", fontWeight: 700, opacity: !denom ? 0.6 : 1 }}>
            {t.rewards.confirm}
          </button>
        </div>
      </div>
  ...
  return (
    <div role="dialog" aria-modal="true" onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 70, background: "rgba(0,0,0,0.4)", display: "grid", placeItems: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 720, background: BRAND.surface, borderRadius: 16, border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 20px 40px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ opacity: 0.7 }}>Ref</div><div style={{ fontWeight: 700 }}>{item.ref}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>Date</div><div>{date}</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>Reward</div><div>{item.brand} — {item.title} ({formatPKR(item.denomination)})</div></div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><div style={{ opacity: 0.7 }}>{t.rewards.points}</div><div>{item.points}</div></div>
...
const PayTab: React.FC<PayTabProps> = ({ t, payments, addPayment, updatePayment, role, utm, onOpenReceipt }) => {
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button onClick={downloadCSV} style={{ padding: "12px 14px", borderRadius: 12, border: `1px solid ${BRAND.ring}`, background: BRAND.surface, color: BRAND.primary, fontWeight: 700 }}>{t.pay.csv}</button>
        </div>
